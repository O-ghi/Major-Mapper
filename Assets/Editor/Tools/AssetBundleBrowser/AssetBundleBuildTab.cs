...
namespace UnityEngine.AssetBundles
{
    ...
    public class AssetBundleBuildTab
    {
        ...
        public class ToggleData
        {
            ...
            {
                if ( onToggles != null && onToggles.Contains(title))
                    state = true;
                else
                    state = s;
        ...
        public void OnGUI(Rect pos)
        {
            ...
            GUILayout.Label(new GUIContent("Packaged game resources"), centeredStyle);
            //basic options
            ...
            // build.
            EditorGUILayout.Space();
            if (GUILayout.Button("Build (Contains packaged lua resources)") )
            {
        ...
        private void ExecuteBuild()
        {
			...
			if (m_CopyToStreaming.state)
			{
				string destDirName = Path.Combine(m_streamingPath, m_UserData.m_BuildTarget.ToString());


				DirectoryCopy(m_UserData.m_OutputPath, destDirName);
			}
        ...
        {
            //string json = File.ReadAllText("Assets/StreamingAssets/clientConfig.json");
            string json  = Resources.Load<TextAsset>("Streaming/clientConfig").text;
			...
			string platform = m_UserData.m_BuildTarget.ToString();

			CryptTools.CreateDir(destDirName, true);

			CryptTools.FileCopy(sourceDirName, destDirName, platform);
			// CryptTools.FileCopy(sourceDirName, destDirName, platform + ".manifest");

			CryptTools.DirectoryCopyEncrypt(sourceDirName, destDirName, (string filename, long loadedsize, long toltalsize) =>
			{
				if (toltalsize > 0)
				{
					float progress = (float)loadedsize / (float)toltalsize;

					EditorUtility.DisplayProgressBar("Copy Assets", filename, progress);
				}
				else
        ...
        public class BuildTabData
        {
            ...
            public ValidBuildTarget m_BuildTarget = ValidBuildTarget.Windows;
