...
public class WWWLoader
{
    ...
    public void Download(string path, System.Action<string, bool, byte[]> callback, System.Action<string, float, ulong> onUpdate, string version = null, bool fromCache = false, bool priority = false)
    {
        ...
        if (!loadingMap.ContainsKey(path))
        {
            ...
            if (nowCorNum < MaxCorNum && loadingList.Count > nowCorNum)
    ...
    public void DownLoad(string path, System.Action<object> callback)
    {
        System.Action<string, bool, byte[]> callFun = (string p, bool b, byte[] arr) =>
        {
            if (b)
                callback(arr);
            else
        ...
        Download(path, callFun, null, TimeUtils.currentMilliseconds() + "");
    }
    ...
    private IEnumerator loadwww(string path)
    {
        ...
        if (info != null)
        {
            ...
            if (info.useCache && UseCache)
            {
                if (cacheVersionMap.ContainsKey(file) && cacheVersionMap[file] == info.version)
                {
                    if (System.IO.File.Exists(cachePath + file))
                    {
                        ...
                        www = UnityWebRequest.Get("file://" + cachePath + file);

#else
                        www = UnityWebRequest.Get(cachePath + file);
#endif
            ...
            if (www == null)
            {
                ...
                {
                    if (path.Contains("history") || path.Contains("/server/"))
                    {
                        www = UnityWebRequest.Get(path);
                    }
                    else
                    {
                        www = UnityWebRequest.Get(path + "?" + info.version);
                    }
                }
            }
            Debuger.Log("Send request URL " + www.url);
            ...
            while (true)
            {
                if (!www.isDone || !Mathf.Approximately(www.downloadProgress, progress))
                {
                    //进度在变化或者还没有执行,重置
                ...
                {
                    //自定义超时
                    Debuger.Err("自定义超时>", TimeOut, path);
                    if (loadingMap.ContainsKey(path))
                ...
                try
                {
                    if (info.updateCb != null)
                        info.updateCb(path, www.downloadProgress, 0);//info.updateCb(path, www.progress, www.bytesDownloaded);
                }
                catch (System.Exception e)
                {
                    Debuger.Err(e.Message, e.StackTrace);
                ...
                else if (www.isDone)
                {
                    ...
                    {
                        if (info.cmpCb != null)
                            info.cmpCb(path, true, www.downloadHandler.data);
                    }
                    catch (System.Exception e)
                    {
                        Debuger.Err(e.Message, e.StackTrace);
                    ...
                    //写缓存
                    if (UseCache && !isFromCache && info.useCache && www.downloadHandler.data.Length <= MaxCacheFileSize)
                    {
                        ...
                        try
                        {
                            fs.BeginWrite(www.downloadHandler.data, 0, www.downloadHandler.data.Length, (ar) => {
                                fs.EndWrite(ar);
                                fs.Flush();
                                fs.Close();
                                fs.Dispose();
