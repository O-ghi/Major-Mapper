...
public class TimeUtils
{
    ...
    {
        return new DateTime(1970, 1, 1, 0, 0, 0).AddMilliseconds(currentMilliseconds()).AddHours(7);
    }

    /// <summary>
    /// 服务器时间转本地C#时间
    /// </summary>
    public static DateTime currentServerToLocalTime()
    {
        var date = new DateTime(1970, 1, 1, 0, 0, 0).AddMilliseconds(currentMilliseconds());
    ...
    /// java时间转C#时间
    /// </summary>
    public static DateTime javaTimeToCSharpTime(long javaMiniSeconds, bool isAdd = true)
    {
        if (isAdd)
            return new DateTime(1970, 1, 1, 0, 0, 0).AddMilliseconds(javaMiniSeconds).AddHours(7);
        else
            return new DateTime(1970, 1, 1, 0, 0, 0).AddMilliseconds(javaMiniSeconds).AddHours(0);
    }
    ...
    /// <param name="time"></param>
    /// <returns></returns>
    public static string FormatTime3(int time, string format = "{0}:{1}")
    {
        int hour = time / 3600;
        int min = (time / 60) % 60;
        string sHour, sMin;
        ...
        return string.Format(format, sHour, sMin);
    }
    ...
    {
        return FormatTime4(time, "天");
    }

    public static string FormatTime4(int time, string strDay)
    {
        ...
        return string.Format("{0}{4}:{1}:{2}:{3}", day, sHour, sMin, sSecond, strDay);
    }
}