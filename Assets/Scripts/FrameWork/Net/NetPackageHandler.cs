        ...
        try
        {
            ...
            while(m_sendOnceSize < MAX_SEND_SIZE)
            {
                ...
                else
                {
                    ...
                    //消息id可能异常重新获取，最多5次(ILRunTime多线程问题)
                    int id = 0;
                    for (int i = 0; i < 5; ++i)
                    {
                        id = entry.msg.GetMsgId();
                        if (id > 0)
                            break;
                        else
                    ...
                    if (id > 0)
                    {
                        bytes = encodePackage(id, entry.msg.GetMsgData(), entry.msg.GetMsgSize(), out size);
                    }
                    else
                    {
#if ENABLE_IL2CPP || UNITY_EDITOR //ILRuntime
                        //var ad = entry.msg as ILRuntime.ILBaseMessageAdapter.Adaptor;
                        //if (ad == null)
                        //    UnityEngine.Debug.LogError("消息id异常 > 消息不是一个ILBaseMessageAdapter.Adaptor > " + id);
                        //else
                        //{
                        //    if (ad.instance == null)
                        //        UnityEngine.Debug.LogError("消息id异常 ad.instance == null >" + id);
                        //    else
                        //        UnityEngine.Debug.LogError("消息id异常 > " + ad.instance.Type.FullName + " >" + id);
                        //}
#endif
                    }
                }
